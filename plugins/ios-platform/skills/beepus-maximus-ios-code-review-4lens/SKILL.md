---
name: code-review-4lens
description: "Four-lens parallel code review for Swift/SwiftUI: reuse, quality, efficiency, clarity. Launches 4 agents scanning the same diff through different lenses, then aggregates and fixes. Triggers on: review code, code review, simplify code, check code quality, 4-lens review, review my changes."
---

# 4-Lens Code Review

Four parallel agents review the same diff through different lenses. Catches code issues that visual review (`review-ui`) and linting miss: duplication, unnecessary state, hot-path work, naming drift.

**Report + fix by default.** Unlike `review-ui` (report-only), this skill applies fixes directly, skipping false positives.
...
1. If $ARGUMENTS specifies files or a diff command, use that
...
If a feature spec or plan exists (check the project specifications or requirements documents and `$ARGUMENTS` for references):
...
- **Do not overlap with review-ui.** This skill reviews code quality. `review-ui` reviews visual design, accessibility, HIG compliance, and anti-slop. Do not flag spacing values, colour choices, animation curves, or typography -- those belong to `review-ui`.
