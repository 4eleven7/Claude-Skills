...
- Apple's XCTest-to-Swift-Testing migration guidance is authoritative for migration details. Read `references/xctest-migration.md` before migrating existing XCTest files.
...
11. **Migration preserves intent.** When removing `import XCTest`, add `import Foundation` if the file relied on XCTest re-exporting Foundation. Convert `XCTAssert*` to `#expect`, `XCTUnwrap` to `#require`, and move `setUp`/`tearDown` into `init`/`deinit` only when the suite shape supports it.
...
- `references/xctest-migration.md` -- Apple-authoritative XCTest-to-Swift-Testing migration guidance and assertion mappings.
