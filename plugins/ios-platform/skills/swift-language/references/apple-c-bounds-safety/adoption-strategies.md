...
1. Before staging anything, run `git status` and `git diff` to enumerate **all** working-tree changes. This includes both the agent's edits and any further edits the user made while the stop was open. Do not assume the working tree contains only what the agent wrote.
...
3. Check the result against the commit's declared scope (stated at each commit site below — e.g. "source-code only", "build-system only", or "headers + validation file"):
    - If every changed file fits the scope, stage exactly those files (agent's + user's) and commit.
