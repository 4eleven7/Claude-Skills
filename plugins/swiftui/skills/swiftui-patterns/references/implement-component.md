...
- NOT for polishing existing code (use `swiftui-patterns` design-quality banlist) or reviewing (use `swiftui-visual-review`)
...
7. Design-memory file located via `references/design-memory.md` — persistent design decisions
8. The feature spec in the project specifications or requirements documents if one exists

Also use:
- `references/component-selection.md` — to choose the right components for what the user wants
- `platform-hig` — to make design decisions
- `swiftui-patterns` design-quality banlist — to load the banlists before writing any code

## Workflow

### Step 1: Understand the Component
Parse the user's request and identify:
- What component(s) are needed (use `references/component-selection.md`)
...
Using `platform-hig`, resolve key design questions:
...
If a design-memory file exists, check for relevant prior decisions. Follow them unless the user explicitly overrides.
...
2. **Components**: Use the patterns from `references/component-selection.md`
...
4. **Anti-Slop**: Mentally check every item from `swiftui-patterns` design-quality banlist banlist as you write
   - No uniform padding
...
4. **Anti-Slop Checklist**: Run the 15-item checklist from `swiftui-patterns` design-quality banlist
...
If significant design decisions were made, offer to update the located design-memory file.
