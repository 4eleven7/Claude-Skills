...
**Owns:** SwiftUI view code — Apple-authoritative SwiftUI patterns, state management, composition, layout, animation, polish, performance, accessibility, navigation, sheets, lists, scrolling, Liquid Glass adoption, SDK 27 migration, and design quality.
...
- **Apple guidance wins.** For topics covered in `references/apple-swiftui-specialist/` or `references/apple-sdk-27/`, those references override local heuristics and model memory.
...
- **Prefer MV over MVVM.** Views multi-agent-implementation; models and services own logic. Favour `@State`, `@Environment`, `.task`, `.onChange` for coordination.
...
- **SDK 27 migration is reference-first.** For `@State` macro compile errors, SDK 27 toolbar APIs, reorderable containers, AsyncImage caching, swipe actions outside `List`, item-binding alerts/dialogs, document apps, or ContentBuilder issues, read `references/apple-sdk-27/` before suggesting fixes. The obvious `@State` init-reordering fix is wrong for SDK 27.
...
1. Check Apple-authoritative SwiftUI topic refs first:
   - view structure → `references/apple-swiftui-specialist/structure.md`
   - data flow and Observation → `references/apple-swiftui-specialist/dataflow.md`
   - environment usage → `references/apple-swiftui-specialist/environment.md`
   - modifiers → `references/apple-swiftui-specialist/modifiers.md`
   - localization → `references/apple-swiftui-specialist/localization.md`
   - `ForEach` identity → `references/apple-swiftui-specialist/foreach.md`
   - soft-deprecated APIs → `references/apple-swiftui-specialist/soft-deprecation.md` and `references/apple-swiftui-specialist/soft-deprecated-apis.md`
2. Check SDK 27-specific changes if compile errors, new APIs, or deprecations are involved → `references/apple-sdk-27/`
3. Check property wrapper usage → `references/state-management.md`
4. Check view composition and structure → `references/composition-and-structure.md`
5. Check performance patterns → `references/performance.md`
6. Check navigation and sheet wiring → `references/navigation-and-sheets.md`
7. Check list and scroll patterns → `references/list-and-scroll.md`
8. Check accessibility → `references/accessibility-patterns.md`
9. Check animation and polish → `references/polish-and-animation.md`
10. Check design quality (AI tells, spacing, typography) → `references/design-quality.md`
11. Check Liquid Glass usage (if iOS 26+) → `references/liquid-glass.md`
12. Check WebKit integration (if embedding web content) → `references/webkit-integration.md`
13. Check rich text editing patterns (if using TextEditor + AttributedString) → `references/styled-text-editing.md`
14. Check advanced toolbar features (if customizable toolbars) → `references/toolbar-advanced.md`
15. Run against the Review Checklist below
...
Use `references/swiftui-patterns.md` for component-level TDD workflow details.
...
- `references/apple-swiftui-specialist/` — Apple-authoritative SwiftUI structure, data flow, environment, modifiers, localization, animations, `ForEach`, and soft-deprecation guidance.
- `references/apple-sdk-27/` — Apple-authoritative SDK 27 SwiftUI changes: `@State` macro, reorderable containers, AsyncImage caching, toolbar overflow/minimization, item-binding alerts/dialogs, swipe actions outside `List`, document APIs, ContentBuilder changes, and deprecations.
