...
**Does NOT own:** StoreKit 2 code, transaction listeners, receipt handling (see `storekit`). Paywall UI implementation (see `swiftui-patterns` or `screen-design-exploration`). App Store description and screenshots (see `app-store-marketing`).
...
- Paywall UI: see `screen-design-exploration`
```
...
- `app-store-release` — submission compliance for IAP-bearing builds.
