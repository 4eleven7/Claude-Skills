---
name: ios-energy
description: Use when diagnosing battery drain, thermal issues, power impact, polling, timers, Low Power Mode behavior, hot devices, Power Profiler findings, or energy efficiency regressions.
---
...
**Does NOT own:** Location API details (see ios-core-location), background task scheduling mechanics (see ios-background-tasks), networking API patterns (see ios-networking), hang diagnosis (see ios-hang-diagnostics), general performance profiling with Time Profiler (see ios-tooling), GPU render pipeline optimization (see ios-metal).
