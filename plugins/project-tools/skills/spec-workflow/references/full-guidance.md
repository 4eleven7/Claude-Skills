...
Use this skill to draft, clarify, research, or sync a feature specification. Pick the smallest mode that fits the request.

## Modes

| Need | Mode | Reference |
|---|---|---|
| Feature request is ambiguous or underspecified | Clarify | Current workflow below |
| User wants a new implementation-ready spec | Draft | `references/spec-workflow.md` |
| User wants research questions for a feature or decision | Research | `references/spec-workflow.md` |
| Implementation changed and spec must be reconciled | Sync | `references/spec-workflow.md` |
...
- When `project-state-audit` skill flags a spec as needing work before implementation
- When the user wants to harden a draft spec before approving it
- When `project-state-audit` skill reports underspecification findings
...
| Want to validate spec quality further | `project-state-audit` skill | Cross-artifact consistency check |
| Spec has drifted from existing code | `spec-workflow` skill | Reconcile with reality first |
| Multiple specs need clarification | `spec-workflow` skill again | Run on the next spec |
| Gap is deferred — worth revisiting later | `capture-idea` skill | Capture the idea with a trigger condition so it surfaces when relevant work begins |
