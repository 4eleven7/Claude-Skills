...
- A workflow (`implement` skill, `ship-it` skill, etc.) produced unexpected results
...
Use the user request, selected files, command arguments, or current repo context as input. For Claude command wrappers, `$ARGUMENTS` — What to investigate. Can be a feature name, a time range, a symptom, or a question.
...
| **Stale artifacts** | Todoproject-scope skill don't match code | Documents not updated after work |
...
| Spec needs updating | `sync-spec` skill |
| Documents out of date | `project-scope` skill |
| Feature incomplete | `implement` skill or `patch` skill |
| Tests missing | `patch` skill to add tests |
```
...
- Keep the report factual and concise — this feeds into other commands that will do the actual fixing
