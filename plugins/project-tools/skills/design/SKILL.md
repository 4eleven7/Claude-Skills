...
Use before creative implementation when the user needs a design direction approved. Use `spec-workflow` for specification drafting and synchronization, `screen-design-exploration` for SwiftUI visual variants, and `implement` only after the design/spec direction is approved.
...
5. **Write spec** — Locate the project's existing specification directory and save the spec using its naming convention. If no specification location exists, ask where specs should live.
...
Specs go to the project's located specifications directory, following the project's existing spec format. Search for existing specs before choosing a path. Read an existing spec in that directory to match the style. If no spec directory or example exists, ask before creating one.
