...
- After `capture-idea` skill — quick gut-check on whether an idea resonates before it sits in the backlog
- Before `implement` skill — validate that a feature will land with real users before investing build time
- During `project-state-audit` skill — prioritise the backlog through persona lenses when multiple features compete
- On demand — evaluate any idea, spec section, or design question against user archetypes

## Input

Use the user request, selected files, selected text, or current repo context as input. The input should be one of:
...
If the input is empty or unclear, ask what the user wants evaluated.
...
This skill can be suggested or auto-triggered by other skills and workflows:

- **`capture-idea` skill** — Runs a lightweight persona review automatically and embeds a 3-sentence verdict in the seed. `user-test` skill can still be run separately for the full detailed report.
- **`project-state-audit` skill** — When recommending next work from competing options, suggest: "Run `user-test` skill to compare these candidates through persona lenses?"
...
- **`next-step-router` skill** — When the user is between tasks and considering what to build, suggest `user-test` skill if the decision involves user-facing features
...
|---|---|---|
| High-scoring idea that's just a seed | `capture-idea` skill (if not already planted) | Capture it properly with trigger conditions |
...
| Comparing backlog items for prioritisation | `project-state-audit` skill | Feed persona rankings into scope decisions |
| Idea surfaced a UX concern | `spec-workflow` skill | Resolve the concern before building |
