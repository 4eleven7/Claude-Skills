---
name: dont-do-that-again
description: Use when a mistake, gap, correction, or near-miss should be captured immediately as a durable lesson, rule, or project instruction.
---

# Don't Do That Again

Capture a mistake, gap, or near-miss from the current session with root-cause analysis and route it to the right knowledge layer. This is a hot-take capture triggered in the moment — not a full session review (use the `session-distill` skill for that).
...
| Project instructions (the project instructions or the project instructions) | Project conventions, architecture decisions, build rules | Append to relevant section |
...
- The rule is already documented — check lessons.md, project instructions, memory, and system docs first
...
3. For lessons.md: prepend to the file (newest first), use the **Mistake** / **Pattern** / **Rule** structure
...
- Include the date (today's date) for lessons.md entries
