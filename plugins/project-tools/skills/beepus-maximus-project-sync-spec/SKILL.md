---
name: sync-spec
description: Reconcile a specification with the implementation after code, review, or product decisions change.
---
...
- When `project-scope` skill flags a spec/code mismatch
...
| "The code is wrong, not the spec" | That's an `implement` skill problem, not a `sync-spec` skill problem. Flag it but do not change working code. |
...
|-----------|-------------------|-----|
| Code is verified and ready to ship | `ship-it` skill | Commit, PR, monitor, land |
...
| Aspirational behaviour worth capturing for later | `idea-seed` skill | Capture the idea with a trigger condition rather than inflating the spec |
| Unbuilt behaviour is minor/isolated | `patch` skill | Small targeted fix without full `implement` skill cycle |
