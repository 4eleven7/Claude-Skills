---
name: project-scope
description: Reconcile repository code, specs, scope, and todos to determine current project state and next work.
---
...
Use this skill as the Scope workflow. Your job is to answer "where are we?" and "what should we build next?" by reconciling the repository, specifications, and project documents.
...
| Spec exists but may have gaps | `clarify-spec` skill | Harden the spec before implementation |
| Spec exists but has drifted from code | `sync-spec` skill | Reconcile before building more |
...
| Multiple specs may be inconsistent | `docs-audit` skill | Cross-artifact consistency check before proceeding |
| Small remaining work on a partial feature | `patch` skill | Targeted fix with verification, no full `implement` skill cycle |
| Idea surfaced during assessment | `idea-seed` skill | Capture for later with trigger conditions |
