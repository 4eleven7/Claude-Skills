...
Use this skill as the Code Style workflow. Your job is to catch and fix style violations that automated tools miss and that agents tend to get wrong by default.

Formatters and linters handle mechanical formatting. This skill handles the patterns where generated code drifts from the project's house style — the things that make code look generated rather than authored.
...
These are patterns agents often get wrong by default. Automated linters do not catch them.
...
| All three quality passes done | `pr-shipping` skill | Commit, PR, monitor, land |
| Style fix is a trivial one-liner | `small-change` skill | Zero-ceremony change with lint verification |
