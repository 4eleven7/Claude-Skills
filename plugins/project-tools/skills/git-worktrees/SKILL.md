---
name: git-worktrees
description: Use when starting isolated feature work in a git worktree or when implementation needs branch/worktree separation.
---
...
Use for branch/worktree isolation decisions and setup. Use `implement` for the actual feature work, `pr-shipping` for commit/push/PR flow, and `forensics` when existing worktree state is confusing.
...
```bash
rg -i "worktree.*director" AGENTS.md README.md docs .agents 2>/dev/null
```
...
**Called by:** `design` (after approval), `multi-agent-implementation` (before parallel tasks), `plan` (before execution)

**Pairs with:** `post-implementation-qa` (for cleanup after work is complete)
