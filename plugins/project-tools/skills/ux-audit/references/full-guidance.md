...
For SwiftUI applications, use the SwiftUI-specific discovery patterns in `references/swiftui-workflow-review.md` as supporting detail. Do not use a separate SwiftUI ux-audit skill.
...
| UX findings require spec changes | `spec-workflow` skill | Reconcile spec with UX reality |
| Ready to ship | `pr-shipping` skill | Commit, PR, monitor, land |
