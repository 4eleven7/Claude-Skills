...
> **Quick Ref:** 5-layer UI workflow audit: discover entry points → trace flows → detect issues → evaluate UX → verify data wiring. Output: `.ux-audit/` in project root.
...
| Command | Description |
|---------|-------------|
| `/ux-audit` | Full 5-layer audit |
| `/ux-audit layer1` | Discovery only — find all entry points |
| `/ux-audit layer2` | Trace — trace critical paths |
| `/ux-audit layer3` | Issues — detect problems across codebase |
| `/ux-audit layer4` | Evaluate — assess user impact |
| `/ux-audit layer5` | Data wiring — verify real data usage |
| `/ux-audit trace "A → B → C"` | Trace a specific user flow path |
| `/ux-audit diff` | Compare current findings against previous audit |
| `/ux-audit fix` | Generate fixes for found issues |
| `/ux-audit status` | Show audit progress and remaining issues |
...
> **Note:** These paths are relative to the installed skill directory. When reading these files, resolve from the skill's installed location, not the current working directory.
...
Run all 5 layers sequentially, outputting findings to `.ux-audit/` in the project root
...
1. Read existing `.ux-audit/layer3-results.yaml` and `.ux-audit/handoff.yaml`
...
3. **One-line next step** — suggest using the `plan` skill if fixes are needed
...
```
To generate a phased fix plan from these findings, use the `plan` skill with the UX audit findings as input.
```
...
After completing all layers (full audit) or `fix` mode, generate `.ux-audit/handoff.yaml` for consumption by the planning skill.
...
```yaml
# Handoff Brief — generated by ux-audit
# Consumed by the plan skill for UX audit follow-up
