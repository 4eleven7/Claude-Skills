...
Use this skill to capture or consume session state so work can resume after compaction, interruption, or a new session.

For resume mode, use `references/resume.md`.
...
## Resume With
`session-handoff` skill — or manually: read this file, then continue from "Remaining Work" step 1.
```
...
**Next step:** <what session-handoff skill will pick up>

To continue: `session-handoff` skill (after compaction or in a new session)
```
