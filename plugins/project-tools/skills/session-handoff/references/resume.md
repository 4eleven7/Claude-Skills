...
If the file does not exist, tell the user there is no paused session to resume and suggest `next-step-router` skill or `project-state-audit` skill instead.
...
- If pausing again: `session-handoff` skill will overwrite it
