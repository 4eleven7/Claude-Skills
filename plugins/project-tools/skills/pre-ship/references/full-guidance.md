...
Also use relevant supporting skills if they apply, such as `platform-hig`, `swiftui-performance-audit`, `swiftui-visual-review`, `ux-audit`, `accessibility-audit`, or `code-review`.
...
| Spec gaps were found during polish | `spec-workflow` skill | Reconcile the spec with reality |
| All quality gates passed | `pr-shipping` skill | Commit, PR, monitor, land |
| Polish fix is minor and isolated | `small-change` skill | Quick targeted fix with verification |
| Spotted an idea for later improvement | `capture-idea` skill | Capture for when this area is revisited |
