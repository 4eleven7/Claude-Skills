---
name: ship-it
description: Commit, push, open pull requests, monitor checks, and land changes through a disciplined shipping workflow.
---

# Ship It — Commit, PR, Monitor, Land
...
Use this skill as the Ship It workflow. You own the entire flow from uncommitted changes to merged PR. There is no handoff — you commit, create the PR, monitor CI, fix failures, and land on main.
...
**Override:** If the user explicitly says `ship-it --direct` or "push to main", skip branch creation and commit directly to main. This is the only way to bypass branch+PR. The default is always branch+PR.
...
**Plan-aware grouping:** If a plan exists for this work (from `implement` skill or `patch` skill), prefer one commit per plan task. Each task in the plan maps to a logical unit of work — use that as the primary grouping, then apply the type+concern strategy within tasks that are too large for a single commit.
...
| Timeout (60 minutes) | `pause-session` skill to checkpoint, then report status and stop |
