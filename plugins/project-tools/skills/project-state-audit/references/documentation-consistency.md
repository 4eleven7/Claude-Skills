...
- When `project-state-audit` skill reveals discrepancies that need deeper investigation
...
### Recommended Actions
1. [Action] — addresses findings #N, #N — suggested command: `spec-workflow` skill or `spec-workflow` skill
2. ...
```
...
|---|---|---|
| Specs have gaps or ambiguity | `spec-workflow` skill | Resolve underspecification interactively |
| Specs have drifted from code | `spec-workflow` skill | Reconcile spec with reality |
| Scope/todo is misaligned | `project-state-audit` skill | Reconcile project status documents |
...
| Finding reveals a deferred idea worth capturing | `capture-idea` skill | Capture for later with trigger conditions |
